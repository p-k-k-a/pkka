...
`packages/domain` may import generated enum objects from `@pkka/api` (e.g. `EventType.ONLINE`)
and should type every enum-valued param with the generated type, never `string`. It must not
use the generated hooks, fetchers or the mutator, and `@pkka/api` must never import from
`@pkka/domain` — that would be a cycle.
