...
the `@plugin` in `app/globals.css` carries the Klub Alumnów values (see
`alumni-design-system`) — no manual color overrides on a freshly-added component.
...
- **Never hand-write hex/px colors.** Use the token classes:
  `bg-primary text-primary-foreground`, `bg-band text-band-foreground`,
...
`@tanstack/react-query`. `lib/query-client.ts` already wires the client via
`createQueryClient()` from `@pkka/api`;
...
`@pkka/domain` holds the logic shared with the mobile app: Polish label maps
(faculty, study type, event type, application status), date formatting, role
predicates, profile contact/URL rules, application form rules. Look there
before writing a label map or a date helper; if a new one would suit both
apps, add it there rather than in `lib/`. `@pkka/theme` owns the colour
tokens — see the `alumni-design-system` skill.

## Gotchas

- Don't bypass `cn()` — you lose tailwind-merge's conflict resolution.
- Don't add a component's colors by hand — fix `packages/theme/design-tokens.ts`
