...
public class UserPrincipalExtractor {
    ...
    public Optional<UserPrincipalInfo> extract(Authentication authentication) {
        return switch (authentication) {
            case OAuth2AuthenticationToken t when t.getPrincipal() instanceof OidcUser u ->
                    Optional.of(new UserPrincipalInfo(
                            u.getSubject(), u.getGivenName(), u.getFamilyName()));
    ...
    public record UserPrincipalInfo(
            String keycloakId,
            String firstName,
            String lastName) {}
}
