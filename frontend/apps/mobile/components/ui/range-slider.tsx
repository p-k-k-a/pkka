...
// Dual-thumb range slider driven by PanResponder — no native slider dependency,
// so the emulator's existing binary needs no rebuild. Values are controlled by
// the parent; gestures are tracked by delta (gesture.dx) so no page-offset
...
export function RangeSlider({ min, max, low, high, step = 1, onChange }: RangeSliderProps) {
  ...
  const lowRef = React.useRef(low);
  const highRef = React.useRef(high);
